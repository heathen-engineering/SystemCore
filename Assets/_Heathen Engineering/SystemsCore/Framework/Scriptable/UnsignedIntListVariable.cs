...
namespace HeathenEngineering.Scriptable
{
    ...
    /// </list>
    /// </example>
    [CreateAssetMenu(menuName = "List Variables/Unsigned Int List")]
    public class UnsignedIntListVariable : DataVariable<List<uint>>
    {
        ...
        [HideInInspector]
        public List<ChangeEventListener<List<uint>>> Listeners = new List<ChangeEventListener<List<uint>>>();

        public override List<uint> Value
        {
            get
            {
                return this.value;
        ...
        public override object ObjectValue
        {
            ...
            set
            {
                if (value.GetType() == typeof(List<uint>))
                    SetValue((List<uint>)value);
            }
        }

        public override void SetValue(List<uint> value)
        {
            if (this.value != value)
            {
                this.value = value;
        ...
        public override void SetValue(DataVariable<List<uint>> value)
        {
        ...
        public override void AddListener(ChangeEventListener<List<uint>> listener)
        {
            Listeners.Add(listener);
        }

        public override void RemoveListener(ChangeEventListener<List<uint>> listener)
        {
            Listeners.Remove(listener);
        }
    }
